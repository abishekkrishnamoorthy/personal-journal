...
const Post = ({ post }) => {
  ...
  return (
    <Card className='post'>
      ...
      <CardActions>
        <IconButton size="medium" color="#0b29166e" onClick={handleLikeClick}>
          {isLiked ? <FavoriteIcon style={{ fill: 'vilot' }} /> : <FavoriteBorderIcon />}
        </IconButton>
        <Link to={`/post/${post._id}`}><Button size="small" color="primary">
          Read More
        </Button></Link>
      </CardActions>
    </Card>
